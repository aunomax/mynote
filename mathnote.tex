...
\end{proposition}
\begin{proof}
	2. Let $x\in\mathbb R^n$ be a Lebesgue point. 
	For any $\varepsilon > 0$, there exists $R>2|x|$ such that 
	\[
	\int_{|y|\geq R\sqrt t} K(y)\dd y + \int_{|x-y|\geq R\sqrt t}|f(y)|\dd y < \varepsilon.
	\]
	Therefore,
	\begin{align*}
		K*f-f &= \int_{\mathbb{R}^n}K(x-y)f(y)\dd y - f(x) = \int_{\mathbb R^n}K(x-y)(f(y)-f(x))\dd y\\
		&\leq \int_{|x-y|<R\sqrt t}K(x-y)|f(y)-f(x)|\dd y + \int_{|x-y|\geq R\sqrt t}K(x-y)|f(y)-f(x)|\dd y\\
		&\leq CR^n\fint_{B_{R\sqrt t}(x)}|f(y)-f(x)|\dd y 
			+ \int_{|x-y|\geq R\sqrt t}|f(y)|\dd y 
			+ |f(x)|\int_{|x-y|\geq R\sqrt t}K(x-y)\dd y,
	\end{align*}
	which implies
	\[
	\limsup_{t\searrow0}|K*f-f| \leq (1+|f(x)|)\varepsilon,
	\] 
	and the pointwise convergence is attained by putting $\varepsilon\searrow0$.

	3. norm convergence and almost-everywhere convergence imply the strong convergence.
	
	We give a direct proof below. 
	For any $\varepsilon>0$, there exists $\delta >0$ such that for any $|y|<\delta$,
	\[
	\|f(x+y)-f(x)\|_{L^1(\mathbb R^n)}<\varepsilon.
	\]
	Since there exists $g\in C_0^{\infty}(\mathbb R^n)$ such that 
	\[
	\|g(x)-f(x)\|_{L^1(\mathbb R^n)}<\varepsilon/3,
	\]
	by density.
	By uniform continuity of $g$, 
	there exists $\delta>0$ such that for any $|y|<\delta$,
	\[
	\|g(x+y)-g(x)\|_{L^1(\mathbb{R}^n)}<\varepsilon/3.
	\]
	we have 
	\begin{align*}
		...
		< \varepsilon.
	\end{align*}
	for $\varepsilon>0$ given above, there exists $R>0$ such that 
	\[
	...
	\begin{align*}
		...
		&= (4\pi)^{-n/2}\int_{\mathbb{R}^n\times\mathbb R^n}e^{-z^2/4}|(f(x-\sqrt tz)-f(x))|\dd x\dd z\\
		...
		&\leq 2\|f\|_1\varepsilon + \varepsilon,\quad\text{provided } \sqrt t<\delta/R.
	\end{align*}
\end{proof}
...
\begin{remark}
	...
	&\leq \liminf_{n\to\infty}\||f_n|+|f|\|_1 - \limsup_{n\to\infty}\|f_n-f\|_1\\
...
\begin{proof}
	...
	Using 
	\[
	\frac{f(\lambda a + (1-\lambda)b) - f(a)}{\lambda a + (1-\lambda)b - a} \leq \frac{f(b) - f(a)}{b-a},
	\]
	we have if $a<b$,
	\[
	...
	\]
	and hence $f'(a+)$ exists and satisfies 
	\[
	f'(a+)\leq \frac{f(b) - f(a)}{b-a}\quad b>a.
	\]
	If $a>b$, we similarly obtain $f'(a-)$ exists and satisfies 
	\[
	f'(a-)\leq \frac{f(c) - f(a)}{c-a}\quad c<a.
	\]
	Noting 
	\begin{align*}
