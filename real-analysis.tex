...
对在$E_X\subset\mathbb{R}$上的有界可测函数$f(x)$，有$\forall\delta>0$，存在闭集$F\subset E_X$满足$m(E_X\backslash F)<\delta$和$\mathbb{R}$上的连续函数$g(x)$满足$g(x)=h(x),\forall x\in E_X\backslash F$。进一步，
$$\inf_{\mathbb{R}}g(x)=\inf_Fh(x)\geq-1,\sup_{\mathbb{R}}g(x)=\sup_Fh(x)\leq1.$$
于是
\begin{align*}
|\int_{\mathbb{R}}f(x)h(x)\mathrm{d}x|&=|(\int_{\mathbb{R}\backslash E_X}+\int_{E_X\backslash F}+\int_F)f(x)h(x)\mathrm{d}x|\\
&=|\int_{\mathbb{R}}f(x)g(x)\mathrm{d}x+(\int_{E_X\backslash F}+\int_{\mathbb{R}\backslash E_x})f(x)(h(x)-g(x))\mathrm{d}x|\\
&\leq(\int_{E\backslash F}+\int_{\mathbb{R}\backslash E_X})|f(x)||h(x)-g(x)|\mathrm{d}x\\
...
f_k(x)\leq f(x)\quad a.e.x\in\mathbb{R}^n.
\end{equation}
另一方面，$\forall\varepsilon>0$，$\exists N>0$，使得$\forall k>N$，有
$$|\int_E(f(x)-f_k(x))\mathrm{d}x<\varepsilon.$$
由(\ref{lebesgue_intergal_2})知$$\int_E|f(x)-f_k(x)|\mathrm{d}x=|\int_E(f(x)-f_k(x))\mathrm{d}x,$$即
$$\lim_{k\to\infty}\int_E|f(x)-f_k(x)|\mathrm{d}x=0.$$
由此可知，$$f_k(x)\Rightarrow f(x).$$
而且应用Fatou引理，$$\int_E\liminf_{k\to\infty}f(x)-f_k(x)\mathrm{d}x=\int_Ef(x)\mathrm{d}x-\int_E\limsup_{k\to\infty}f_k(x)\mathrm{d}x\leq0.$$


从而
$$\int_Ef(x)\mathrm{d}x\leq\int_E\limsup_{k\to\infty}f_k(x)\mathrm{d}x\leq\int_Ef(x)\mathrm{d}x.$$
即$\limsup_{k\to\infty}f_k(x)=f(x),a.e.x\in E.$
